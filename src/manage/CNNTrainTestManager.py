...
class CNNTrainTestManager(object):
    ...
    def __init__(self, model,
        ...
        self.checkpoint_path = checkpoint_path if checkpoint_path is not None else f"best_{self.model.__class__.__name__}.pt"
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            val_acc = self.metric_values['val_acc'][-1]
            if val_acc > best_val_acc:
                best_val_acc = val_acc
                self.model.save('best_model.pt')
                print("Checkpoint saved (val_acc: {:.3f})".format(best_val_acc))
    ...
    def evaluate_on_validation_set(self):
        ...
        current_val_acc = float(np.mean(validation_accuracies))
        if current_val_acc > self.best_val_acc:
            self.best_val_acc = current_val_acc
            torch.save(self.model.state_dict(), self.checkpoint_path)
            print(f"Saved best checkpoint to {self.checkpoint_path} (val_acc={current_val_acc:.3f})")

        # displays metrics
        current_lr = self.optimizer.param_groups[0]['lr']
        print('Validation loss %.3f' % (validation_loss / len(val_loader)))
        print('Validation accuracy %.3f' % (current_val_acc))
        print('Current learning rate %.6f' % current_lr)
