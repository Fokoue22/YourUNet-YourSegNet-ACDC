...
class RandomSegmentationAugmentation:
    """Apply the same geometric augmentation to an image and its mask."""

    def __call__(self, img_slice, gt_slice):
        ...
        if k:
            img_slice = np.rot90(img_slice, k, axes=(0, 1))
            gt_slice = np.rot90(gt_slice, k, axes=(0, 1))

        return img_slice.copy(), gt_slice.copy()
...
class HDF5Dataset(data.Dataset):
    ...
    def __init__(self, set, file_path, input_size: int = 256, transform=None, augment=False):
        """
        Args:
        """
        ...
        self.augmenter = RandomSegmentationAugmentation() if augment else None
    ...
    def __getitem__(self, index):
        ...
        if self.augmenter is not None:
            img_slice, gt_slice = self.augmenter(img_slice, gt_slice)
        ...
        if self.transform is not None:
            ...
            # fix a random seed (use a value that fits in int32)
            seed = np.random.randint(0, 2147483647)  # Max int32 value
            # print(seed)
